 ...
 * Scroll-pinned Flip across 4 layouts on desktop & mobile.
 * Pin lives on the section; entrance reveal is opacity-only on the wrapper
 * so transforms never fight ScrollTrigger pin on touch devices.
 */
  ...
  useEffect(() => {
      ...
      flipTween = Flip.from(state, {
        ...
        onEnter: (elements) => {
          gsap.fromTo(
            elements,
            { opacity: 0, scale: 0.9 },
            { opacity: 1, scale: 1, duration: isMobile ? 0.3 : 0.4, ease: 'power2.out', overwrite: 'auto' }
          );
        },
        onLeave: (elements) => {
          gsap.to(elements, {
    ...
    const ctx = gsap.context(() => {
      ...
      const setupPinnedFlip = (isMobile: boolean) => {
        ...
        const pinDistance = () => `+=${Math.round(window.innerHeight * (isMobile ? 2.6 : 3))}`;

        // Opacity-only entrance — no y transform on the pinned subtree
        gsap.fromTo(
          wrapper,
          { autoAlpha: 0 },
          {
        ...
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            const targetIdx = Math.min(3, Math.floor(self.progress * 4));
  ...
  if (!mounted) {
    return (
      <div className="w-full h-[100svh] bg-black">
        <section id="services-section" className="w-full h-full bg-black" />
      </div>
    );
  }

  return (
    <div ref={wrapperRef} className="flip-section-wrapper w-full">
      <section
        id="services-section"
        ref={sectionRef}
      ...
      >
        <style>{`
          ...
          /* ── Mobile layouts (<992px) — mirror desktop 4 flip states ── */
