...
export default function Testimonials() {
  ...
  return (
    <section id="testimonial-section" className="w-full bg-[#ffffff] text-black py-12 sm:py-20 md:py-24 overflow-hidden select-none">
      {/* ─── HEADER SECTION ─── */}
      ...
      >
        <div ref={trackRef} className="inline-flex gap-3.5 sm:gap-8 will-change-transform">
          {slides.map((card, idx) => (
            <div
              key={`${card.id}-${idx}`}
              className="w-[78vw] xs:w-[82vw] sm:w-[620px] md:w-[740px] shrink-0 bg-white rounded-[20px] sm:rounded-[32px] p-4 xs:p-5 sm:p-8 flex flex-col md:flex-row gap-3.5 sm:gap-8 items-center border border-gray-200/80 shadow-sm transition-all duration-300 hover:shadow-md hover:border-gray-300"
            >
              {/* Text Info Container */}
              <div className="flex-1 flex flex-col justify-between h-full text-left w-full">
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2.5 sm:mb-4">
                    ...
                    <span className="font-sora font-bold text-sm sm:text-base md:text-lg text-black tracking-tight">
                      {card.brand}
                    </span>
                  </div>

                  {/* Quote */}
                  <blockquote className="font-inter text-xs xs:text-sm sm:text-base text-gray-800 leading-snug sm:leading-relaxed mb-3 sm:mb-8 font-normal">
                ...
                {/* Author Info */}
                <div>
                  <h3 className="font-sora font-bold text-xs sm:text-base md:text-lg text-black leading-tight">
                    {card.name}
                  </h3>
                  <p className="font-inter text-[10px] sm:text-sm text-gray-500 font-medium mt-0.5">
                    {card.position}
                  </p>
                  <p className="font-inter text-[9px] sm:text-xs text-gray-400 mt-0.5">
                    {card.location}
                  </p>
                </div>
              </div>
              ...
              {/* Portrait Image Side */}
              <div className="w-full md:w-[250px] lg:w-[280px] h-[140px] xs:h-[160px] sm:h-[300px] shrink-0 relative rounded-lg sm:rounded-2xl overflow-hidden border border-black/5">
                <Image
                  src={card.image}
                  alt={card.name}
                  fill
