...
export default function ThreeDRotator() {
  ...
  function getColor(cubeIdx: number, brightnessMultiplier: number): string {
    ...
    const r = Math.round(brandColors[lo][0] + (brandColors[hi][0] - brandColors[lo][0]) * frac);
    const g = Math.round(brandColors[lo][1] + (brandColors[hi][1] - brandColors[lo][1]) * frac);
    const b = Math.round(brandColors[lo][2] + (brandColors[hi][2] - brandColors[lo][2]) * frac);
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 2. Timeline for individual cube rotations and brand color shifting
      ...
      // 3. Tray movement, rocking, and scaling timelines
      gsap.timeline()
