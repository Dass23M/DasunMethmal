...
export default function Testimonials() {
  ...
  return (
    <section id="testimonial-section" className="w-full bg-[#080808] text-white py-12 sm:py-20 md:py-24 overflow-hidden select-none">
      {/* ─── HEADER SECTION ─── */}
      <div className="max-w-[1440px] mx-auto px-4 sm:px-12 lg:px-20 mb-6 sm:mb-16 flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-8">
        <div>
          ...
          <h2 className="font-sora font-black text-2xl sm:text-4xl md:text-5xl lg:text-6xl text-white tracking-tight leading-tight">
            Trusted Brands Worldwide
          </h2>
        </div>
        <p className="font-inter text-xs sm:text-sm md:text-base text-white/70 max-w-md leading-relaxed">
      ...
      >
        <div ref={trackRef} className="inline-flex gap-3.5 sm:gap-8 will-change-transform">
          {slides.map((card, idx) => (
            <div
              key={`${card.id}-${idx}`}
              className="w-[78vw] xs:w-[82vw] sm:w-[620px] md:w-[740px] shrink-0 bg-[#121212] rounded-[20px] sm:rounded-[32px] p-4 xs:p-5 sm:p-8 flex flex-col md:flex-row gap-3.5 sm:gap-8 items-center border border-white/10 shadow-sm transition-all duration-300 hover:shadow-md hover:border-white/20"
            >
              {/* Text Info Container */}
              <div className="flex-1 flex flex-col justify-between h-full text-left w-full">
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2.5 sm:mb-4">
                    ...
                    <span className="font-sora font-bold text-sm sm:text-base md:text-lg text-white tracking-tight">
                      {card.brand}
                    </span>
                  </div>

                  {/* Quote */}
                  <blockquote className="font-inter text-xs xs:text-sm sm:text-base text-white/90 leading-snug sm:leading-relaxed mb-3 sm:mb-8 font-normal">
                ...
                {/* Author Info */}
                <div>
                  <h3 className="font-sora font-bold text-xs sm:text-base md:text-lg text-white leading-tight">
                    {card.name}
                  </h3>
                  <p className="font-inter text-[10px] sm:text-sm text-white/70 font-medium mt-0.5">
                    {card.position}
                  </p>
                  <p className="font-inter text-[9px] sm:text-xs text-white/40 mt-0.5">
                    {card.location}
                  </p>
                </div>
              </div>
              ...
              {/* Portrait Image Side */}
              <div className="w-full md:w-[250px] lg:w-[280px] h-[140px] xs:h-[160px] sm:h-[300px] shrink-0 relative rounded-lg sm:rounded-2xl overflow-hidden border border-white/10">
                <Image
                  src={card.image}
                  alt={card.name}
                  fill
